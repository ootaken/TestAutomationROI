...
テスト部門外の欠陥コストについては、発生する欠陥の頻度、重要度によって異なり、自動化された後の値は同一の規模や特性を持ったソフトウェアからのベンチマークになるため、精確な予想値を求めるのが困難な場合があります。このため、簡易的なテスト部門内に完結したROIの試算では使用しない場合もあります。
...
TABOK の3つのROIの試算式を見てきましたが、最後の Risk Reduction ROI Method で取り扱った間接コストについて考えてみましょう。製品あるいは本番サービスでの発生した障害の対応コストについて、欠陥の修正コストについては、開発部門と(分離してれば)品質管理部門が負うことになりますが、クレーム対応やリコール対応にかかるコストはカスタマーサポート部門、広報部門、営業部門などの複数に及ぶこともあります。このような自部門外で発生するコストを除外して自部門のROI改善のみに注力すると局部最適化になってしまう可能性があります。
